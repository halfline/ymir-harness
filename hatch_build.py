...
def _ensure_ui_workflows_submodule(root: Path) -> None:
    checkout = root / "ui-workflows"
    if _looks_like_ymir_checkout(checkout):
        return

    if not (root / ".git").exists():
        message = (
            "ui-workflows is not initialized; run "
            "`git submodule update --init ui-workflows` before syncing"
        )
        raise RuntimeError(message)

    completed = subprocess.run(
        ["git", "-C", str(root), "submodule", "update", "--init", "ui-workflows"],
        check=False,
        text=True,
        capture_output=True,
    )
    if completed.returncode != 0:
        ...
        message = "failed to initialize ui-workflows submodule"
        if details:
            message = f"{message}: {details}"
        raise RuntimeError(message)

    if not _looks_like_ymir_checkout(checkout):
        raise RuntimeError("ui-workflows submodule did not provide the ymir package")
