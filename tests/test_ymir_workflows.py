        ...
        json.dumps(
            {
                ...
                "data": {
                    ...
                    "justification": "generated by triage",
        ...
        json.dumps(
            {
                ...
                "data": {"jira_issue": "RHEL-12345"},
            }
        ),
        encoding="utf-8",
    )

    result = _backport_inputs(request)

    assert isinstance(result, RunCaseExecution)
    assert result.status == "failed"
    assert "triage result is not backport" in (result.reason or "")
